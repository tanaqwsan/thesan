...
--[[
if getBot().name == "JBlGTqGqpT" or getBot().name == "xrGzXv1DyP" or getBot().name == "p3D4rYg8PO" then
    logging = true
else
    logging = false
end
]]
function getCapitalOf(text)
    return string.upper(text)
end
function getx()
    if pcall(function()
        math.floor(getBot().x) end) then
        return math.floor(getBot().x)
    else
        return "nilx"
    end
end
function gety()
    if pcall(function()
        math.floor(getBot().y) end) then
        return math.floor(getBot().y)
    else
        return "nily"
    end
end
...
function ws(text)
    ...
    reqGet("http://172.104.212.103/ws.php?msg="..text)
end
function w(text)
    if logging then
        file = io.open("logs\\"..getBot().name..".txt", "a")
...
function join(a,b)
    ...
    if b==nil then
        ...
        sleep(500)
        if world.name ~= "EXIT" then
            if world.name==a and getBot():getWorld():getTile(math.floor(getBot():getWorld():getLocal().posx/32),math.floor(getBot():getWorld():getLocal().posy/32)).fg ~= 6 then
                w("join 2")
                return
            end
        end
        ...
        w("join 7")
        if world.name ~= "EXIT" then
            if getBot():getWorld().name == a and getBot():getWorld():getTile(math.floor(getBot():getWorld():getLocal().posx/32),math.floor(getBot():getWorld():getLocal().posy/32)).fg == 6 then
                w("join 9")
                return
            end
        end
...
function jalanJalan()
    ...
    join("EXIT")
    sleep(2000)
    theRW = randomWorlds[random(1,#randomWorlds)]
    join(theRW)
    w("going to "..theRW)
    sleep(4000)
    w("world now : "..getBot():getWorld().name)
    x = 7 + nil
end
...
function getLocalIp()
    w("getLocalIp 1")
    if localip1=="" or localip1:find("Bad") or localip1:find("error") then
        w("getLocalIp 2")
        ...
        localip1 = httpResult.body
    end
    w("getLocalIp 3")
    if localip2=="" then
        w("getLocalIp 4")
        localip2 = getBot():getProxy().ip
    end
    w("getLocalIp 5")
    return localip2
    --[[
...
function rq(state)
    ...
    httpClient = HttpClient.new()
    httpClient:setMethod(Method.get)
    httpClient.url = ipBotStatus..'?growid='..getBot().name..'&world='..getBot().level.."|"..getBot():getWorld().name.."__"..tostring(ribu(getBot().gem_count))..'&ip='..getLocalIp()..'&state='..state
    httpClient.headers["User-Agent"] = "Lucifer"
    httpClient:request()
end
...
function main()
    ...
    if pcall(main0) then
        ...
        rq("Bot_disconnected..")
        sleep(2000)
        if getBot().status == 0 or
        getBot().status == 7 or
        getBot().status == 8 or
        getBot().status == 10 or
        getBot().status == 16 or
        getBot().status == 19 then
            w("main 2")
            sleep(10000)
            while getBot().status ~= 1 do
                if getBot().status == 2 then
                    rq("wrong_password")
                    sleep(4000)
                    rq("wrong_password")
                    sleep(4000)
                ...
                elseif getBot().status == 15 then
                    rq("error_connecting_"..menit.."m")
                    sleep(menit * 60000)
                    return main()
                elseif getBot().status == 17 then
                    rq("captcha_requested")
                    sleep(4000)
                    rq("captcha_requested")
                    sleep(4000)
                    sleepTrue()
                elseif getBot().status == 18 then
                    rq("mod_entered_"..menit.."m")
                    sleep(menit * 60000)
                    return main()
                end
                w("main 3")
                rq("Bot_reconnecting.."..conke)
                if getBot().status ~= 1 then
                    getBot():connect()
                end
                sleep(10000)
                if conke == 12 then
                    rq("recon_fail_reconnecting,sleep_"..menit.."m.."..conke)
                    sleep(menit * 60000)
                    conke = 0
                    w("main 4")
                end
